...
function bestIntervention() {
  return derivedDecisionCandidates.reduce((winner, candidate) => calculateDecisionScore(candidate).score > calculateDecisionScore(winner).score ? candidate : winner)
}

function baselineIntervention() {
  return derivedDecisionCandidates.find((candidate) => candidate.id === "do-nothing") ?? derivedDecisionCandidates[0]
}
...
function FocusConfiguration({ state, buildings, onSelectBuilding, onUpdateScenario, onSimulate, onOpenOperations, onReset }: Omit<FocusModeProps, "onChooseScenario" | "onOpenDecision">) {
  ...
  return <section className="focus-config" data-testid="focus-config" aria-labelledby="focus-config-title">
    ...
    <div className="focus-form">
      ...
      <label><span>Duration</span><select aria-label="Focus duration" value={state.scenario.durationHours} onChange={(event) => onUpdateScenario({ durationHours: Number(event.target.value) })} disabled={state.phase === "running"}>{configuration.durationOptions.map((hours) => <option key={hours} value={hours}>{hours} hours</option>)}</select></label>
    ...
    <details className="focus-advanced"><summary>Advanced options <span>EDIT INPUTS</span></summary><div className="focus-advanced-grid">
      <label><span>Start time</span><select aria-label="Focus start time" value={state.scenario.startTime} onChange={(event) => onUpdateScenario({ startTime: event.target.value })}><option value="09:00">09:00</option><option value="10:00">10:00</option><option value="11:00">11:00</option><option value="12:00">12:00</option><option value="15:00">15:00</option></select></label>
      {configuration.advancedExactSeverity ? <label className="focus-range"><span>Exact severity <output>{severity}%</output></span><input aria-label="Focus exact severity" type="range" min="20" max="100" step="1" value={severity} onChange={(event) => onUpdateScenario({ severity: Number(event.target.value) })} /></label> : null}
      <fieldset><legend>Affected systems</legend>{affectedSystems.map((system) => <label key={system.id}><input type="checkbox" checked={systems.includes(system.id)} onChange={() => toggleSystem(system.id)} /><span>{system.label}</span></label>)}</fieldset>
    </div></details>
    <div className="focus-submit-row"><div><span className="focus-eyebrow">READY TO TEST</span><p>{state.scenario.targetLabel ?? "Selected target"} / {state.scenario.durationHours}h / {severity}% severity</p></div><button className="focus-primary-button" type="button" onClick={onSimulate} disabled={state.phase === "running" || state.phase === "complete"}>Simulate <span>→</span></button></div>
...
function FocusSimulation({ state }: { state: CampusState }) {
  ...
  const sequence = [configuration.sourceEventLabel, "Classes displaced", "Room pressure", "Student movement", "Faculty conflict", "Transport load", "Campus stability"]
  return <section className="focus-simulation" data-testid="focus-simulation" aria-live="polite" aria-labelledby="focus-simulation-title">
    <div className="focus-eyebrow">RIPPLE / SIMULATING</div><h1 id="focus-simulation-title">{state.scenario.targetLabel ?? "TT Block"} · {state.scenario.durationHours} hours</h1><p>The deterministic model is revealing the consequence sequence.</p><div className="focus-progress"><span style={{ width: `${Math.max(8, ((active + 1) / 7) * 100)}%` }} /></div><div className="focus-progress-label"><span>Simulation running</span><span>{active < 0 ? "Starting" : `${Math.min(active + 1, 7)} of 7 consequences`}</span></div><div className="focus-sequence">{sequence.map((item, index) => <span className={index <= active ? "active" : ""} key={item}>{item}</span>)}</div>
  </section>
}
...
function FocusResult({ state, panel, setPanel, onOpenOperations, onOpenDecision, onReset }: { state: CampusState; panel: FocusPanel; setPanel: (panel: FocusPanel) => void; onOpenOperations: FocusModeProps["onOpenOperations"]; onOpenDecision: FocusModeProps["onOpenDecision"]; onReset: FocusModeProps["onReset"] }) {
  const best = bestIntervention()
  const nothing = baselineIntervention()
